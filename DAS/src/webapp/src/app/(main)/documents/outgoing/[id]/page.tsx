...
export default function OutgoingDocDetail() {
  const [activeTab, setActiveTab] = useState<'content' | 'approval' | 'versions' | 'ai'>('content');
  const [comments, setComments] = useState([
    { user: 'Lê Kế Toán Viên', text: 'Đã hoàn tất soạn thảo dự thảo quyết định bổ nhiệm nhân sự kế toán.', date: '18/07/2026 14:30' },
    { user: 'Nguyễn Kế Toán Trưởng', text: 'Đã duyệt qua nội dung dự thảo. Trình Ban Giám đốc ký số đóng dấu.', date: '18/07/2026 16:15' }
  ]);
  ...
  const doc = {
    docNo: 'CV-DI-2026-00089',
    subject: 'Quyết định bổ nhiệm nhân sự phòng kế toán',
    receiver: 'Nội bộ / Phòng Kế toán',
    docDate: '18/07/2026',
    creator: 'Lê Kế Toán Viên',
    priority: 'Thường',
    status: 'Đã phát hành',
    content: 'Quyết định bổ nhiệm bà Nguyễn Văn A giữ chức vụ phó trưởng phòng kế toán doanh nghiệp kể từ ngày 19/07/2026...'
  };
  ...
  return (
    <>
      <Header title="Chi tiết công văn đi" />
            ...
            <div className="space-y-6">
              <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-medium">
                  <div>
                    <span className="text-muted-foreground block mb-1">Nơi nhận:</span>
                    <span className="font-bold text-foreground">{doc.receiver}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-muted-foreground block mb-1">Ngày phát hành:</span>
          ...
          {activeTab === 'approval' && (
            <div className="p-6 bg-card border border-border rounded-2xl space-y-8">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative">
                {[
                  { step: 1, title: 'Soạn thảo', desc: 'Thư ký phòng Kế toán', status: 'Đã hoàn thành', date: '18/07 14:30', done: true },
                  { step: 2, title: 'Kiểm duyệt', desc: 'Trưởng phòng Kế toán', status: 'Đã duyệt', date: '18/07 16:15', done: true },
                  { step: 3, title: 'Phê duyệt & Ký số', desc: 'Giám đốc Công ty', status: signed ? 'Đã ký số' : 'Chờ phê duyệt', date: signed ? '19/07 11:00' : 'Đang xử lý', done: signed }
                ].map((s, idx) => (
            ...
            <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { version: 'v1.0 (Dự thảo)', size: '1.1 MB', date: '18/07/2026 14:30', author: 'Lê Kế Toán Viên', current: !signed, note: 'Bản soạn thảo ban đầu trình ký.' }
                ].map((v, i) => (
            ...
            <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
              <div className="flex justify-between items-center border-b border-border pb-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xl font-extrabold text-emerald-650 dark:text-emerald-450">99.2%</div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left font-semibold">
                  ...
                  <tbody className="divide-y divide-border/60">
                    {[
                      { field: 'Số hiệu công văn', reg: 'CV-DI-2026-00089', ai: 'CV-DI-2026-00089', match: true },
                      { field: 'Nơi nhận', reg: 'Nội bộ / Phòng Kế toán', ai: 'Phong Ke toan', match: true },
                      { field: 'Ngày phát hành', reg: '18/07/2026', ai: '18/07/2026', match: true },
                      { field: 'Người ký', reg: 'Nguyễn Văn Director', ai: 'Nguyen Van Director', match: true }
                    ].map((row, idx) => (
            ...
            <div className="w-40 h-40 border border-border rounded-xl bg-white mx-auto flex items-center justify-center p-3 overflow-hidden shadow-sm">
              <img 
                src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
                  typeof window !== 'undefined' ? window.location.href : 'http://localhost:3000/documents/outgoing/1'
                )}`} 
                alt="QR Code" 
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/75 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-card border border-border w-full max-w-sm rounded-2xl shadow-xl overflow-hidden transition-all duration-200 p-6 space-y-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground font-semibold">Nhập mã PIN của Chứng thư số CA cá nhân để hoàn tất quy trình phê duyệt công văn đi.</p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className="w-full px-3 py-2 border border-border bg-muted rounded-xl text-xs focus:outline-none text-foreground font-semibold">
                  ...
                  <option>Thư ký Nguyễn Kế Toán Trưởng (CA-2026-DAS)</option>
                </select>
              </div>
