...
export default function InternalDocDetail() {
  ...
  const [comments, setComments] = useState([
    { user: 'Trần Văn Trưởng Phòng', text: 'Đã hoàn tất dự thảo thông báo nghỉ phép tập thể năm 2026. Trình Ban Giám đốc phê duyệt.', date: '17/07/2026 10:30' },
    { user: 'Nguyễn Văn Director', text: 'Thống nhất lịch nghỉ từ ngày 01/08 đến hết ngày 05/08. Ký ban hành toàn công ty.', date: '17/07/2026 11:15' }
  ]);
  ...
  const doc = {
    docNo: 'CV-NB-2026-00034',
    subject: 'Thông báo lịch nghỉ phép tập thể năm 2026',
    department: 'Phòng Hành chính',
    docDate: '17/07/2026',
    creator: 'Trần Văn Trưởng Phòng',
    priority: 'Thường',
    status: 'Đã phân phối',
    content: 'Thông báo lịch nghỉ mát hè và nghỉ phép tập thể năm 2026 dành cho toàn bộ cán bộ công nhân viên công ty DAS, thời gian kéo dài 5 ngày kể từ ngày 01/08/2026...'
  };
  ...
  return (
    <>
      <Header title="Chi tiết công văn nội bộ" />
      <main className="flex-1 p-8 grid grid-cols-1 lg:grid-cols-4 gap-8 max-w-7xl mx-auto w-full">
            ...
            <div className="space-y-6">
              <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
                <div className="flex justify-between items-center border-b border-border pb-4">
                  ...
                  <span className="px-3 py-1 bg-emerald-100 text-emerald-800 dark:bg-emerald-950/20 dark:text-emerald-450 rounded-full text-xs font-bold">
                    {doc.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-medium">
                  <div>
                    <span className="text-muted-foreground block mb-1">Đơn vị chủ trì:</span>
                    <span className="font-bold text-foreground">{doc.department}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block mb-1">Người soạn thảo:</span>
                    <span className="font-bold text-foreground">{doc.creator}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block mb-1">Ngày phân phối:</span>
                    <span className="text-foreground">{doc.docDate}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block mb-1">Độ khẩn:</span>
                    <span className="px-2 py-0.5 bg-muted text-foreground rounded text-[10px] font-bold">
                      {doc.priority}
                    </span>
                  </div>
                </div>
              ...
              <div className="p-6 bg-card border border-border rounded-2xl space-y-4">
                ...
                <div className="w-full h-[500px] border border-border rounded-xl bg-muted overflow-hidden relative">
                  <iframe 
                    src="/documents/samples/internal-sample.pdf"
          ...
          {activeTab === 'approval' && (
            <div className="p-6 bg-card border border-border rounded-2xl space-y-8">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative">
                {[
                  { step: 1, title: 'Soạn thảo', desc: 'Thư ký Hành chính', status: 'Đã hoàn thành', date: '17/07 10:30', done: true },
                  { step: 2, title: 'Kiểm duyệt', desc: 'Trưởng phòng Hành chính', status: 'Đã duyệt', date: '17/07 11:00', done: true },
                  { step: 3, title: 'Phê duyệt & Ký số', desc: 'Giám đốc Công ty', status: signed ? 'Đã ký số' : 'Chờ phê duyệt', date: signed ? '17/07 11:15' : 'Đang xử lý', done: signed }
                ].map((s, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center text-center p-4 border border-border bg-muted/50 rounded-xl relative w-full">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs mb-3 ${
                      s.done 
                        ...
                        : 'bg-zinc-250 text-zinc-600 dark:bg-zinc-850 dark:text-muted-foreground'
                    }`}>
                      {s.step}
                    </div>
          ...
          {activeTab === 'versions' && (
            <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
              ...
              <div className="space-y-4">
                {[
                  { version: 'v2.0 (Bản ký số)', size: '1.4 MB', date: '17/07/2026 11:15', author: 'Giám đốc Nguyễn Văn Director', current: signed, note: 'Bản chứa chữ ký số và con dấu đỏ điện tử.' },
                  { version: 'v1.0 (Dự thảo)', size: '1.3 MB', date: '17/07/2026 10:30', author: 'Trần Văn Trưởng Phòng', current: !signed, note: 'Bản soạn thảo ban đầu trình ký.' }
                ].map((v, i) => (
            ...
            <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
              <div className="flex justify-between items-center border-b border-border pb-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xl font-extrabold text-emerald-650 dark:text-emerald-450">98.8%</div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left font-semibold">
                  ...
                  <tbody className="divide-y divide-border/60">
                    {[
                      { field: 'Số hiệu công văn', reg: 'CV-NB-2026-00034', ai: 'CV-NB-2026-00034', match: true },
                      { field: 'Đơn vị soạn thảo', reg: 'Phòng Hành chính', ai: 'Phong Hanh Chinh', match: true },
                      { field: 'Ngày phân phối', reg: '17/07/2026', ai: '17/07/2026', match: true },
                      { field: 'Người ký ban hành', reg: 'Nguyễn Văn Director', ai: 'Nguyen Van Director', match: true }
                    ].map((row, idx) => (
        ...
        <div className="space-y-6">
            ...
            <div className="w-40 h-40 border border-border rounded-xl bg-white mx-auto flex items-center justify-center p-3 overflow-hidden shadow-sm">
              <img 
                src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
                  typeof window !== 'undefined' ? window.location.href : 'http://localhost:3000/documents/internal/1'
                )}`} 
                alt="QR Code" 
          ...
          <div className="p-6 bg-card border border-border rounded-2xl space-y-6">
            <h4 className="font-bold text-xs uppercase tracking-wide text-foreground">Ý kiến chỉ đạo & Trao đổi</h4>
            ...
            <div className="space-y-4 overflow-y-auto max-h-64 pr-1">
              {comments.map((c, idx) => (
                <div key={idx} className="space-y-1 text-xs">
                  ...
                  <p className="text-muted-foreground font-medium leading-relaxed bg-muted/40 p-2.5 rounded-lg border border-border/40">{c.text}</p>
                </div>
              ))}
            </div>

            <form onSubmit={handleAddComment} className="pt-4 border-t border-border flex gap-2">
              <input
                type="text"
                value={newComment}
                onChange={(e) => setNewComment(e.target.value)}
                placeholder="Nhập ý kiến chỉ đạo..."
                className="flex-1 px-3 py-2 border border-border bg-muted rounded-xl text-xs focus:outline-none focus:border-accent text-foreground"
              />
              <button
                type="submit"
                className="px-3 py-2 bg-primary text-primary-foreground font-bold rounded-xl text-xs hover:opacity-90 cursor-pointer"
              >
                Gửi
              </button>
            </form>
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/75 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-card border border-border w-full max-w-sm rounded-2xl shadow-xl overflow-hidden transition-all duration-200 p-6 space-y-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground font-semibold">Nhập mã PIN của Chứng thư số CA cá nhân để hoàn tất quy trình phê duyệt công văn nội bộ.</p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className="w-full px-3 py-2 border border-border bg-muted rounded-xl text-xs focus:outline-none text-foreground font-semibold">
                  ...
                  <option>Trưởng phòng Trần Văn Trưởng Phòng (CA-2026-DAS)</option>
                </select>
              </div>
