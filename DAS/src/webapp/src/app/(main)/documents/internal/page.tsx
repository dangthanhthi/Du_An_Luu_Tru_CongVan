...
export default function InternalDocs() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('custom_internal_docs');
      if (saved) {
  ...
  const defaultDocs = [
    { id: '1', docNo: 'CV-NB-2026-00034', subject: 'Thông báo lịch nghỉ phép tập thể năm 2026', author: 'Phòng Hành chính', date: '17/07/2026', priority: 'Thường', status: 'Đã phân phối' },
  ];

  const docs = [...customDocs, ...defaultDocs];

  const filteredDocs = docs.filter((doc) => {
    const authorStr = doc.author || doc.department || 'Phòng Hành chính';
    const matchesSearch = 
  ...
  return (
    <>
      <Header title="Danh sách công văn nội bộ" />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-950 p-4 border border-zinc-800 rounded-xl transition-all duration-200 shadow-sm">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative flex items-center w-full md:w-64">
              <input
                type="text"
                placeholder="Tìm kiếm công văn nội bộ..."
                value={searchQuery}
            ...
            >
              ...
              <option value="Đã ký số ban hành">Đã ký số ban hành</option>
            </select>
          </div>
          
          <Link
            href="/documents/internal/new"
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-zinc-900 text-zinc-500 text-[10px] uppercase font-bold tracking-wider bg-black/40">
                  ...
                  <th className="py-4 px-6">Phòng soạn thảo / Áp dụng</th>
                  <th className="py-4 px-6">Ngày phân phối</th>
                ...
                {filteredDocs.length > 0 ? (
                  filteredDocs.map((doc) => (
                    <tr key={doc.id} className="hover:bg-zinc-900/40 transition-colors">
                      ...
                      <td className="py-4 px-6 text-zinc-200 font-medium hover:underline">
                        <Link href={`/documents/internal/${doc.id}`}>{doc.subject}</Link>
                      </td>
                      <td className="py-4 px-6 text-zinc-400 font-semibold">{doc.author || doc.department || 'Phòng Hành chính'}</td>
                  ...
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-xs text-zinc-500 font-medium">
                      Không tìm thấy công văn nội bộ nào phù hợp.
                    </td>
                  </tr>
                )}
              </tbody>
